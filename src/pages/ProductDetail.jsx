...
export default function ProductDetail() {
  ...
  const renderStars = (rating) => {
    const stars = [];
    for (let i = 1; i <= 5; i++) {
      if (i <= rating) stars.push(<Star key={i} size={16} fill="currentColor" className="text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.6)]" />);
      else if (i - 0.5 <= rating) stars.push(<StarHalf key={i} size={16} fill="currentColor" className="text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.6)]" />);
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#020817] relative overflow-hidden">
        ...
        <motion.div animate={{ rotate: 360 }} transition={{ duration: 2, repeat: Infinity, ease: "linear" }} className="relative z-10 w-24 h-24 flex items-center justify-center">
          <div className="absolute inset-0 border-t-2 border-r-2 border-blue-400 rounded-full opacity-50"></div>
          <div className="absolute inset-2 border-b-2 border-l-2 border-blue-300 rounded-full opacity-70 animate-spin-reverse"></div>
          <Package className="text-[#0866bd]" size={32} />
        </motion.div>
        <p className="font-black tracking-[0.3em] uppercase text-[10px] animate-pulse text-blue-400 relative z-10 mt-6">Cargando Especificaciones...</p>
      </div>
    );
  }
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-slate-400 bg-[#020817]">
  ...
  return (
    <div className="bg-[#020817] min-h-screen selection:bg-[#0866bd] selection:text-white pb-20 relative overflow-hidden">
      
      {/* === AMBIENT BACKGROUND GLOW (Estilo Tech-Dashboard) === */}
      ...
      <AnimatePresence>
        {showStickyBar && product.stock > 0 && (
          <motion.div 
            ...
            className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] w-[90%] sm:w-auto min-w-[350px] max-w-[600px] bg-slate-900/80 backdrop-blur-2xl border border-white/10 p-3 pl-6 rounded-[2.5rem] shadow-[0_30px_60px_rgba(0,0,0,0.6)] flex items-center justify-between gap-6"
          >
            ...
            <div className="flex items-center gap-5 w-full sm:w-auto">
              ...
              <motion.button 
                ...
                className="flex-1 sm:flex-none relative overflow-hidden bg-gradient-to-r from-blue-600 to-[#0866bd] text-white font-black uppercase tracking-[0.1em] rounded-[1.5rem] px-6 py-3.5 shadow-[0_5px_20px_rgba(8,102,189,0.4)] hover:shadow-[0_10px_30px_rgba(8,102,189,0.6)] text-[10px] flex items-center justify-center gap-2 border border-blue-400/30 group"
              >
      ...
      <div className="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-16 relative z-10">
        ...
        <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-8 sm:mb-12 border-b border-white/10 pb-4">
          <Link to="/" className="hover:text-[#0866bd] transition-colors">Inicio</Link> <ChevronRight size={12} className="text-slate-600" />
          <Link to="/catalogo" className="hover:text-[#0866bd] transition-colors">Inventario</Link> <ChevronRight size={12} className="text-slate-600" />
          <span className="text-[#0866bd] truncate max-w-[150px] sm:max-w-none">{product.category}</span>
        </div>

        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
          
          {/* === COLUMNA IZQUIERDA: GALERÍA LEVITANTE & ESCÁNER === */}
          <div className="w-full lg:w-[55%] flex flex-col gap-6 relative lg:sticky lg:top-32 h-fit">
            ...
            <motion.div 
              ...
              className={`bg-slate-900/40 backdrop-blur-2xl rounded-[3rem] flex items-center justify-center border border-white/10 shadow-[0_30px_80px_rgba(0,0,0,0.5)] relative overflow-hidden group flex-1 min-h-[400px] lg:min-h-[600px] w-full aspect-square lg:aspect-auto ${isScannerActive ? 'cursor-default' : 'cursor-none'}`}
            >
               {/* Resplandor focal trasero */}
               <div className="absolute inset-0 bg-gradient-to-tr from-blue-900/20 to-transparent opacity-50 group-hover:opacity-80 transition-opacity duration-500"></div>
               ...
               <motion.img 
                 ...
                 onError={(e) => { e.target.onerror = null; e.target.src = `https://placehold.co/600x600/1e293b/0866BD?text=${encodeURIComponent(product.category)}`; }}
               />
               
               {/* === LUPA FLOTANTE TOP-TIER === */}
               {isZooming && !isScannerActive && (
                 <motion.div
                   className="pointer-events-none absolute z-20 rounded-full border border-white/30 shadow-[0_20px_50px_rgba(0,0,0,0.5),inset_0_0_30px_rgba(8,102,189,0.3)] overflow-hidden bg-slate-900/40 backdrop-blur-md"
                 ...
                 >
                   ...
                   />
                   {/* Retícula de mira del zoom */}
                   <div className="absolute inset-0 flex items-center justify-center opacity-20 mix-blend-overlay pointer-events-none">
                     <div className="w-full h-[1px] bg-white"></div><div className="absolute h-full w-[1px] bg-white"></div>
               ...
               <AnimatePresence>
                 {isScannerActive && (
                   <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-30 bg-[#0866bd]/20 backdrop-blur-[2px] pointer-events-none">
                     <div className="absolute inset-0 bg-[linear-gradient(rgba(8,102,189,0.3)_1px,transparent_1px),linear-gradient(90deg,rgba(8,102,189,0.3)_1px,transparent_1px)] bg-[size:40px_40px]"></div>
                     <motion.div 
                     ...
                     >
                        <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-b from-transparent to-cyan-400/20 -translate-y-full"></div>
                     </motion.div>
                     <div className="absolute top-8 left-8 text-cyan-400 font-mono text-[10px] uppercase tracking-[0.3em] flex flex-col gap-2 drop-shadow-[0_0_10px_rgba(34,211,238,0.8)]">
               ...
               <div className="absolute bottom-8 right-8 flex flex-col gap-3 z-40">
                 <button 
                   ...
                   className={`p-4 rounded-[1.2rem] backdrop-blur-2xl transition-all duration-300 shadow-xl border ${isScannerActive ? 'bg-cyan-500 text-slate-900 border-cyan-300 shadow-[0_0_30px_rgba(6,182,212,0.5)]' : 'bg-slate-900/60 text-slate-400 border-white/10 hover:text-white hover:bg-slate-800 hover:border-white/30 hover:scale-105'}`}
               ...
               {product.isUniversal && (
                 <div className="absolute top-8 right-8 sm:left-8 sm:right-auto bg-slate-900/80 backdrop-blur-md text-yellow-400 text-[10px] font-black px-5 py-2.5 rounded-xl uppercase tracking-[0.2em] shadow-[0_15px_30px_rgba(0,0,0,0.5)] flex items-center gap-2 z-30 border border-white/10">
                   <Zap size={14} className="fill-current animate-pulse drop-shadow-[0_0_8px_rgba(250,204,21,0.8)]" /> Ajuste Universal
                 </div>
               )}
            </motion.div>

            {/* Miniaturas Premium (Glassmorphism) */}
          ...
          <div className="w-full lg:w-[45%] flex flex-col h-fit relative">
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.2, type: "spring" }}>
              ...
              <div className="flex flex-wrap items-center gap-4 mb-10">
                <div className="flex items-center gap-1.5 bg-yellow-500/10 text-yellow-400 px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest border border-yellow-500/30 shadow-[0_0_15px_rgba(250,204,21,0.15)]">
                ...
                <span className="text-[11px] font-black text-slate-500 uppercase tracking-widest bg-slate-900 px-4 py-2 rounded-xl border border-slate-800">SKU: {product.sku}</span>
              </div>
              ...
              {/* === TARJETA DE COMPRA "HARDWARE TITANIO" === */}
              <div ref={buySectionRef} className="mb-10 flex flex-col bg-slate-900/60 backdrop-blur-2xl p-8 sm:p-10 rounded-[3rem] border border-white/10 shadow-[0_30px_60px_rgba(0,0,0,0.6)] relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-80 h-full bg-gradient-to-l from-[#0866bd]/10 to-transparent opacity-60 pointer-events-none transform skew-x-12 translate-x-10 group-hover:bg-[#0866bd]/20 transition-colors duration-700"></div>
                  ...
                  >
                    {addedAnimation ? (
                      ...
                      <div className="flex items-baseline gap-5">
                         <span className="text-[4rem] sm:text-[4.5rem] font-black text-transparent bg-clip-text bg-gradient-to-br from-white to-slate-400 tracking-tighter drop-shadow-2xl leading-none">
                ...
                <div className="flex flex-col sm:flex-row gap-5 w-full relative z-10">
                  <div className="flex items-center bg-slate-950 border border-slate-700 rounded-2xl h-16 sm:w-40 overflow-hidden focus-within:border-[#0866bd] focus-within:shadow-[0_0_20px_rgba(8,102,189,0.3)] transition-all shadow-inner shrink-0">
                     ...
                     <span className="flex-1 text-center font-black text-white text-xl border-x border-slate-800">{qty}</span>
                  ...
                  <motion.button 
                    ...
                    className="relative flex-1 overflow-hidden bg-gradient-to-r from-blue-700 to-[#0866bd] text-white font-black uppercase tracking-[0.2em] rounded-2xl shadow-[0_20px_40px_rgba(8,102,189,0.4)] hover:shadow-[0_25px_50px_rgba(8,102,189,0.6)] transition-all duration-500 text-[11px] sm:text-xs flex items-center justify-center h-16 group disabled:opacity-50 disabled:grayscale border border-blue-400/50"
                  >
                    ...
                    <span className="relative z-10 flex items-center gap-3 drop-shadow-md">
                      {product.stock === 0 ? 'AGOTADO' : <><ShoppingCart size={20} className="group-hover:animate-bounce" /> Añadir a la orden</>}
                    </span>
                  </motion.button>
                </div>
              </div>

              {/* Logística Premium (Dark Mode) */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-10">
                 <div className="bg-slate-900/40 backdrop-blur-md p-6 rounded-[2rem] border border-white/10 shadow-[0_10px_30px_rgba(0,0,0,0.3)] hover:border-[#0866bd]/50 hover:bg-slate-900/60 transition-all group">
                   <div className="bg-blue-500/10 text-[#0866bd] p-3 rounded-xl shrink-0 w-max mb-4 group-hover:bg-[#0866bd] group-hover:text-white transition-colors border border-blue-500/20"><Store size={22}/></div>
                   <div>
                     <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Logística</p>
                     <p className="text-base font-bold text-white mb-0.5">Recolección Inmediata</p>
                     <p className="text-[11px] text-slate-400 font-medium">Sucursal Tonalá, Jalisco.</p>
                   </div>
                 </div>

                 <div className="bg-slate-900/40 backdrop-blur-md p-6 rounded-[2rem] border border-white/10 shadow-[0_10px_30px_rgba(0,0,0,0.3)] hover:border-emerald-500/50 hover:bg-slate-900/60 transition-all group">
                   <div className="bg-emerald-500/10 text-emerald-500 p-3 rounded-xl shrink-0 w-max mb-4 group-hover:bg-emerald-500 group-hover:text-slate-900 transition-colors border border-emerald-500/20"><RefreshCcw size={22}/></div>
                   <div>
                     ...
                     <p className="text-[11px] text-slate-400 font-medium">Devoluciones sin costo</p>
                   </div>
                 </div>
              </div>
      ...
      <div className="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 mt-12 mb-32 relative z-10">
        ...
        <div className="flex border-b border-slate-800 mb-12 overflow-x-auto custom-scrollbar relative">
          {NAV_TABS.map(tab => (
            <button 
            ...
            >
              <tab.icon size={18} strokeWidth={2.5} className={activeTab === tab.id ? 'text-[#0866bd]' : 'opacity-50'} />
              <span>{tab.label}</span>
              {tab.id === 'reviews' && (
                <span className={`px-2.5 py-1 rounded-md text-[9px] font-black transition-colors duration-300 shadow-sm ${activeTab === 'reviews' ? 'bg-[#0866bd] text-white shadow-[0_0_15px_rgba(8,102,189,0.5)]' : 'bg-slate-800 text-slate-400'}`}>
                  {reviews.length}
                </span>
              )}
              {activeTab === tab.id && (
                <motion.div layoutId="active-tab-line" className="absolute bottom-0 left-0 right-0 h-1 bg-[#0866bd] rounded-t-full shadow-[0_-2px_15px_rgba(8,102,189,0.8)]" transition={{ type: "spring", stiffness: 500, damping: 30 }} />
              )}
            </button>
          ))}
        </div>
             ...
             <motion.div key={activeTab} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl">
                   {product.features.map((feat, idx) => (
                      <motion.div 
                        ...
                        className="flex items-start gap-6 p-8 bg-slate-900/50 backdrop-blur-xl rounded-[2rem] border border-white/5 shadow-[0_15px_30px_rgba(0,0,0,0.3)] hover:bg-slate-900/80 hover:border-white/10 transition-all duration-300 group"
                      >
                        <div className="bg-white/5 p-3.5 rounded-xl border border-white/10 group-hover:bg-[#0866bd]/20 group-hover:border-[#0866bd]/50 transition-colors shadow-inner shrink-0">
                          <Check size={20} className="text-[#0866bd] group-hover:drop-shadow-[0_0_8px_rgba(8,102,189,0.8)] transition-all"/>
                        </div>
               ...
               {activeTab === 'compatibility' && (
                 <div className="max-w-4xl">
                    {product.isUniversal ? (
                      <div className="p-12 bg-gradient-to-r from-emerald-900/40 to-slate-900/40 backdrop-blur-2xl text-white rounded-[2.5rem] flex flex-col sm:flex-row items-center gap-10 shadow-[0_30px_60px_rgba(0,0,0,0.5)] relative overflow-hidden border border-emerald-500/20">
                      ...
                      <div className="bg-slate-900/60 backdrop-blur-2xl rounded-[2.5rem] border border-white/10 shadow-[0_30px_60px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col">
                        <div className="p-8 border-b border-white/5 bg-slate-950/50 flex items-center gap-5">
                           ...
                           {tabFilteredCompatibility.length > 0 ? (
                             <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                               {tabFilteredCompatibility.map((c, i) => (
                                 <div key={i} className="flex items-center justify-between p-6 rounded-2xl border border-white/5 bg-white/5 hover:bg-white/10 hover:border-blue-500/30 transition-all group shadow-sm">
                                   ...
                                   <span className="text-[10px] font-black text-[#0866bd] uppercase tracking-widest bg-[#0866bd]/10 px-4 py-2 rounded-xl border border-[#0866bd]/20 shadow-inner group-hover:shadow-[0_0_15px_rgba(8,102,189,0.3)] transition-shadow">
                      ...
                      <div className="px-8 py-16 bg-slate-900/50 backdrop-blur-md rounded-[2.5rem] text-base text-slate-400 font-medium text-center border border-white/10 shadow-sm">Consulta especificaciones técnicas con tu mecánico de confianza o directamente en el mostrador.</div>
                    )}
                 </div>
               )}
               ...
               {activeTab === 'reviews' && (
                 <div className="max-w-5xl flex flex-col md:flex-row gap-12 lg:gap-20">
                   <div className="w-full md:w-1/3 flex flex-col">
                     <div className="bg-slate-900/60 backdrop-blur-2xl p-10 rounded-[3rem] border border-white/10 shadow-[0_30px_60px_rgba(0,0,0,0.5)] text-center flex flex-col items-center">
                       ...
                       <p className="text-[10px] text-slate-500 font-black mb-10 tracking-[0.2em] uppercase">Basado en {reviews.length} calificaciones</p>
                       <button onClick={() => setShowReviewForm(true)} className="w-full bg-white hover:bg-slate-200 text-slate-900 font-black py-5 rounded-[1.5rem] transition-all text-[11px] uppercase tracking-[0.2em] shadow-[0_15px_30px_rgba(255,255,255,0.1)] hover:shadow-[0_20px_40px_rgba(255,255,255,0.2)] flex justify-center items-center gap-3">
                   ...
                   <div className="w-full md:w-2/3">
                     {reviews.length === 0 ? (
                       <div className="text-center py-24 bg-slate-900/40 backdrop-blur-md rounded-[3rem] border border-white/5 shadow-sm">
                       ...
                       <div className="space-y-8">
                         {reviews.map((rev) => (
                           <div key={rev.id} className="bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-8 sm:p-10 shadow-[0_20px_40px_rgba(0,0,0,0.3)] hover:border-[#0866bd]/40 transition-colors group">
                             <div className="flex justify-between items-start mb-6">
                               <div className="flex items-center gap-5">
                                 <div className="w-14 h-14 bg-gradient-to-tr from-[#0866bd] to-blue-500 text-white font-black rounded-2xl flex items-center justify-center text-xl uppercase shadow-inner border border-blue-400/30 group-hover:scale-110 transition-transform duration-500">
                                   {rev.name.charAt(0)}
                                 </div>
                                 <div>
                             ...
                             <p className="text-slate-300 text-sm sm:text-base leading-relaxed font-medium pl-[5rem] relative">
                               <Quote size={24} className="absolute left-4 top-0 text-slate-700 -rotate-180" />
                               {rev.comment}
                             </p>
      ...
      {/* === MODAL DE RESEÑAS (Glassmorphism Oscuro) === */}
      <AnimatePresence>
        {showReviewForm && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, backdropFilter: "blur(12px)" }} exit={{ opacity: 0, backdropFilter: "blur(0px)" }} className="absolute inset-0 bg-slate-950/80" onClick={() => !isSubmittingReview && setShowReviewForm(false)}></motion.div>
            ...
            <motion.div 
              ...
              className="bg-slate-900 w-full max-w-xl rounded-[3rem] relative z-10 shadow-[0_40px_100px_rgba(0,0,0,0.8)] overflow-hidden border border-white/20"
            >
              <div className="px-10 py-8 bg-slate-950 flex justify-between items-center relative overflow-hidden border-b border-white/10">
                <div className="absolute top-[-50%] right-[-10%] w-40 h-40 bg-[#0866bd]/30 rounded-full blur-[40px] pointer-events-none"></div>
                <h3 className="text-sm font-black uppercase tracking-[0.2em] text-white flex items-center gap-3 relative z-10 drop-shadow-md">
                  <div className="bg-yellow-400/10 p-2 rounded-xl border border-yellow-400/20 shadow-inner"><Star size={18} className="text-yellow-400 fill-current drop-shadow-[0_0_8px_rgba(250,204,21,0.8)]"/></div> Tu Opinión Biker
                </h3>
                <button onClick={() => !isSubmittingReview && setShowReviewForm(false)} className="text-slate-500 hover:text-white transition-colors relative z-10 bg-white/5 p-2 rounded-full hover:bg-white/10"><X size={20} strokeWidth={2.5}/></button>
              </div>
              
              <form onSubmit={handleSubmitReview} className="p-10 space-y-8">
                <div className="flex flex-col items-center mb-4">
                  <div className="flex gap-2 bg-slate-950 p-4 rounded-3xl border border-white/10 shadow-inner">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button key={star} type="button" onClick={() => setNewReview({...newReview, rating: star})} className="focus:outline-none transition-transform hover:scale-125 active:scale-90 p-1.5">
                        <Star size={40} fill={star <= newReview.rating ? "#facc15" : "transparent"} strokeWidth={star <= newReview.rating ? 0 : 2} className={star <= newReview.rating ? "text-yellow-400 drop-shadow-[0_0_15px_rgba(250,204,21,0.8)]" : "text-slate-700"} />
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input type="text" required placeholder="Ej. Juan Pérez" value={newReview.name} onChange={(e) => setNewReview({...newReview, name: e.target.value})} className="w-full bg-slate-950 border border-slate-700 rounded-2xl px-6 py-4 text-sm font-bold text-white focus:outline-none focus:border-[#0866bd] focus:bg-slate-900 focus:shadow-[0_0_20px_rgba(8,102,189,0.2)] transition-all placeholder:text-slate-600"/>
                </div>

                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 ml-2">Comentario / Reseña</label>
                  <textarea required rows="4" placeholder="Describe tu experiencia con la calidad y la instalación..." value={newReview.comment} onChange={(e) => setNewReview({...newReview, comment: e.target.value})} className="w-full bg-slate-950 border border-slate-700 rounded-2xl p-6 text-sm font-bold text-white focus:outline-none focus:border-[#0866bd] focus:bg-slate-900 focus:shadow-[0_0_20px_rgba(8,102,189,0.2)] transition-all resize-none placeholder:text-slate-600 leading-relaxed"></textarea>
                </div>

                <button 
                  type="submit" disabled={isSubmittingReview} 
                  className="w-full bg-gradient-to-r from-blue-700 to-[#0866bd] text-white font-black py-5 rounded-2xl uppercase tracking-[0.2em] transition-all text-xs flex items-center justify-center gap-3 disabled:opacity-50 shadow-[0_15px_30px_rgba(8,102,189,0.3)] hover:shadow-[0_20px_40px_rgba(8,102,189,0.5)] border border-blue-500/50 group relative overflow-hidden"
                >
                  <div className="absolute inset-0 bg-[linear-gradient(45deg,transparent_25%,rgba(255,255,255,0.2)_50%,transparent_75%,transparent_100%)] bg-[length:250%_250%,100%_100%] bg-[position:-100%_0,0_0] group-hover:bg-[position:200%_0,0_0] transition-[background-position] duration-[1.5s]"></div>
