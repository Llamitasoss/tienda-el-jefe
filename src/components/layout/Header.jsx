...
export default function Header() {
  ...
  // ENVOLVEMOS TODO EN UN FRAGMENTO (<> ... </>) PARA SEPARAR LOS MODALES DEL HEADER
  return (
    <>
      <motion.header 
        initial={false}
        animate={{ 
          backgroundColor: isScrolled ? 'rgba(8, 102, 189, 0.85)' : 'rgba(8, 102, 189, 1)',
          backdropFilter: isScrolled ? 'blur(16px)' : 'blur(0px)',
          boxShadow: isScrolled ? '0 10px 30px -10px rgba(0,0,0,0.3)' : '0 4px 6px -1px rgba(0,0,0,0.1)'
        }}
      ...
      >
        <motion.div 
          ...
          transition={{ duration: 0.3 }}
          className="max-w-[85rem] mx-auto flex items-center justify-between gap-4"
        >
          {/* LOGO & BRANDING */}
          ...
          <div ref={searchContainerRef} className="hidden md:block relative flex-1 max-w-2xl mx-4 lg:mx-8">
            <motion.form 
              ...
              animate={{ 
                boxShadow: isSearchFocused ? '0 0 0 4px rgba(250,204,21,0.2)' : '0 0 0 0px rgba(250,204,21,0)'
              }}
            ...
            >
              <input 
                ...
                className="w-full py-2.5 px-6 bg-transparent outline-none text-sm font-bold text-slate-800 placeholder:text-slate-400 placeholder:font-medium" 
                autoComplete="off"
              />
              <motion.button 
                ...
                whileTap={{ scale: 0.95 }}
                type="submit" 
                className="bg-gradient-to-r from-[#0866bd] to-blue-600 text-white w-10 h-10 rounded-full flex items-center justify-center shadow-md shrink-0"
              >
            ...
            <AnimatePresence>
              {showDropdown && searchTerm.trim().length >= 2 && (
                <motion.div 
                  ...
                  className="absolute top-[110%] left-0 w-full bg-white/95 backdrop-blur-2xl rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.15)] border border-slate-100 overflow-hidden flex flex-col z-50"
                >
                      ...
                      <div className="flex flex-col p-2">
                        {searchResults.map((prod, idx) => (
                          <motion.div 
                            ...
                            className="flex items-center justify-between p-3 rounded-2xl hover:bg-blue-50/80 cursor-pointer transition-colors group"
                          >
                            ...
                            <p className="text-sm font-black text-slate-900 shrink-0 pl-4 group-hover:text-[#0866bd] transition-colors">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <motion.button 
              whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              onClick={toggleCart} 
              className="relative bg-gradient-to-r from-yellow-400 to-amber-500 text-slate-900 w-12 h-12 rounded-full flex items-center justify-center shadow-[0_5px_15px_rgba(250,204,21,0.3)] hover:shadow-[0_8px_25px_rgba(250,204,21,0.5)] transition-shadow border border-yellow-300"
            >
              ...
              <AnimatePresence>
                {cartCount > 0 && (
                  <motion.span 
                    ...
                    className="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full font-black border-2 border-[#0866bd] shadow-sm"
                  >
                    {cartCount}
                  </motion.span>
                )}
        ...
        <motion.nav 
          ...
          className="hidden lg:flex bg-[#0f172a] border-b border-slate-800 justify-center relative"
        >
          <div className="flex gap-2 p-1" onMouseLeave={() => setHoveredNav(null)}>
            {NAV_LINKS.map((link) => (
              <Link 
                ...
                className="relative px-6 py-4 text-slate-300 font-black text-[11px] uppercase tracking-[0.2em] flex items-center transition-colors hover:text-white z-10"
              >
                {link.name}
                {link.isVIP && (
      ...
      {/* AHORA LOS MODALES ESTÁN FUERA DEL HEADER PARA QUE NO SE CORTEN */}
      <MobileMenu 
        isOpen={mobileMenu} 
